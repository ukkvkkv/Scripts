...
# Форвардинг нужен AmneziaWG: выходная нода маршрутизирует трафик клиентов,
# пришедший вторым прыжком со входной ноды.
...
# ============================================================================
# AmneziaWG 3.0 — второй прыжок каскада
# ============================================================================
# Выходная нода поднимает AWG-сервер на UDP 443. TCP 443 при этом занят Caddy
# (или REALITY, если tproxy отключён) — это разные протоколы, конфликта нет,
# а UDP 443 снаружи выглядит как QUIC и почти не режется операторами.
#
# Отличие 3.0 от 2.0 — HeaderProtectionKey: заголовки пакетов (тип, индекс,
# счётчик) шифруются ChaCha20, тогда как в 2.0 они оставались открытыми и по
# ним DPI уверенно опознавал WireGuard сквозь всю обфускацию.
#
# Входная нода подключается сюда клиентом. Её ключи генерируются ЗДЕСЬ и
# уезжают во входной скрипт одной строкой — так же, как vless-ссылка: иначе
# пришлось бы гонять публичный ключ обратно вручную.
...
install_amneziawg() {
  ...
  # Перезагрузка не нужна: DKMS собирает модуль под работающее ядро сразу,
  # если стоят подходящие заголовки. Если ядро и заголовки разъехались —
  # честно падаем, а не поднимаем половину конфигурации.
...
# Параметры обфускации. Ограничения не косметические, все три из исходников:
#   S1-S4 >= 12  — nonce для ChaCha20 берётся из первых 12 байт S-паддинга,
#                  при меньшем значении модуль отвергает HeaderProtectionKey
#                  (netlink.c: has_protection && val16 < HEADER_PROTECTION_NONCE_SIZE);
#   S1+56 != S2  — иначе init и response совпадут по размеру на проводе;
...
setup_awg_exit() {
  ...
  # umask восстанавливаем: дальше скрипт пишет sshd_config и jail.d, которым
  # права 600 ни к чему.
  ...
  cat > "$conf" <<EOF_AWG
# Выход каскада: сюда приходит второй прыжок со входной ноды и отсюда трафик
# уходит в интернет. Единственный пир — входная нода; клиенты живут на ней.
[Interface]
...
EOF_AWG
  ...
  umask "$old_umask"
  echo "AmneziaWG поднят: ${AWG_PORT}/udp, ${AWG_SUBNET_PREFIX}.1/24, пир входной ноды ${AWG_HOP_IP}"
}

# Всё, что нужно входной ноде для второго прыжка, — одной строкой. Приватный
# ключ пира тоже здесь: его сгенерировала эта нода, чтобы не гонять публичный
# ключ обратно руками.
awg_hop_string() {
  {
    printf 'V=1\n'
...
# UDP 443 под AmneziaWG. С TCP 443 не конфликтует — разные протоколы.
...
# AmneziaWG ставим последним: к этому моменту все TCP-порты разобраны, и видно,
# что UDP 443 действительно свободен.
setup_awg_exit
...
ufw allow 80/tcp
# AmneziaWG: второй прыжок каскада приходит сюда.
...
echo "AmneziaWG: ${AWG_PORT}/udp, каскад через входную ноду (${AWG_HOP_IP})"
echo
...
echo "$EU_VLESS_LINK"
echo
echo "--- Строка второго прыжка AmneziaWG (её же скормить RU-скрипту) ---"
echo "AWGHOP1:$(awg_hop_string)"
echo
echo "В ней ключи и параметры обфускации, включая приватный ключ входной ноды."
echo "Передавать её только по защищённому каналу и никуда не публиковать."
echo
if [[ "$TPROXY_ENABLED" == "1" ]]; then
