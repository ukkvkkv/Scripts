...
# Подписка со ссылками hysteria2:// и vless://. Отдельной подписки с готовым
# JSON-конфигом sing-box больше нет — она не нужна.
...
# Каталог сайта-прикрытия: его отдаёт nginx как dest для REALITY-хендшейков.
# Подписка живёт на том же nginx отдельным location по токену.
...
# Страница-прикрытие: её отдаёт dest для REALITY. Замени на что-нибудь своё —
# одинаковый стартер у многих операторов сам становится приметой при активном
# сканировании.
make_site() {
  local dir="$1"
  install -d -m 755 "$dir"
...
setup_nginx() {
  ...
  # порт и работает как dest для REALITY-хендшейков: всех, кто пришёл без
  # валидного ключа, REALITY прозрачно пробрасывает сюда, и сканер видит
  # обычный сайт с валидным сертом.
  ...
  # Корень отдаёт страницу-прикрытие, подписка живёт отдельным location по
  # токену со своим root. Так сканер видит обычный сайт, а не голый 404.
...
# ============================================================================
# AmneziaWG 3.0 — каскад клиент -> эта нода -> выходная нода
# ============================================================================
# Схема без sing-box вообще: клиент говорит по AmneziaWG с этой нодой (UDP 443,
# TCP 443 при этом у REALITY — разные протоколы, конфликта нет), а заграничный
# прыжок ноды делают между собой интерфейсом awg1.
#
# Отличие 3.0 от 2.0 — HeaderProtectionKey: заголовки пакетов шифруются
# ChaCha20, тогда как в 2.0 тип, индекс и счётчик оставались открытыми.
...
install_amneziawg() {
  ...
  # Перезагрузка не нужна: DKMS соберёт модуль под работающее ядро сразу, если
  # стоят подходящие заголовки. Если они разъехались — падаем честно.
...
# Параметры обфускации для входа "клиент -> эта нода". С параметрами второго
# прыжка они не общие: там свой набор, пришедший в hop-строке.
# Ограничения из исходников модуля:
#   S1-S4 >= 12  — nonce ChaCha20 берётся из первых 12 байт S-паддинга, иначе
#                  netlink вернёт -EINVAL и HeaderProtectionKey не применится;
#   S1+56 != S2  — иначе init и response совпадут по размеру;
#   S3 != S2+28  — иначе совпадут response и cookie reply;
#   H1-H4 различны — это подмена типов сообщений 1..4.
gen_awg_params() {
...
# Разбор строки AWGHOP1:..., которую напечатал скрипт выходной ноды.
# Значения раскладываем через case, а не eval: строка приходит копипастой,
# и выполнять её содержимое как код незачем.
parse_awg_hop() {
...
setup_awg_entry() {
  ...
  # umask восстанавливаем в конце: дальше скрипт пишет sshd_config и jail.d,
  # которым права 600 ни к чему.
  local old_umask; old_umask=$(umask); umask 077

  # --- awg1: второй прыжок, эта нода как клиент выходной ---
  cat > /etc/amnezia/amneziawg/awg1.conf <<EOF_AWG1
# Второй прыжок каскада.
#
# Table = off — критично. Без него awg-quick пропишет дефолтный маршрут и
# уведёт в туннель весь трафик самой ноды, включая SSH, sing-box и nginx.
# Вместо этого политика: в таблицу ${AWG_TABLE_HOP} уходит только подсеть клиентов.
[Interface]
PrivateKey = ${HOP_PRIV}
Address = ${HOP_IP}/32
MTU = 1420
...
# Первый прыжок: сервер для клиентов. MASQUERADE здесь не нужен — он висит
# на awg1 (для трафика в туннель) и добавляется awg-ru-routes на внешнем
# интерфейсе (для российских префиксов, идущих напрямую).
[Interface]
...
EOF_AWG0
  ...
  # Параметры нужны скрипту awg, чтобы новые клиенты получали ровно те же
  # значения обфускации и тот же ключ заголовков.
...
EOF_PARAMS
  ...
  # Рукопожатие со вторым прыжком — единственный признак, что каскад собрался.
  local hs=""
  for _ in {1..10}; do
  ...
  if [[ "${hs:-0}" == "0" ]]; then
    echo "ВНИМАНИЕ: рукопожатия со вторым прыжком (${HOP_ENDPOINT}) пока нет."
    echo "Проверь, что на выходной ноде открыт ${AWG_PORT}/udp и hop-строка свежая."
  else
    echo "Второй прыжок поднят: рукопожатие с ${HOP_ENDPOINT} есть."
  fi
}

# Сплит-роутинг: российские префиксы уходят с этой ноды напрямую, остальное —
# во второй прыжок. Матчинг идёт по IP, а не по доменам: в WireGuard нет слоя,
# который видел бы имена, поэтому .ru на зарубежном хостинге уедет за границу,
# а российский хостинг с доменом .com пойдёт напрямую.
setup_ru_routes() {
  cat > /usr/local/sbin/awg-ru-routes <<'EOF_RU'
#!/usr/bin/env bash
# Российские префиксы в отдельную таблицу маршрутизации.
#
# Таблица 101 содержит ТОЛЬКО российские префиксы, и её правило стоит раньше
# правила таблицы 100 (там дефолт во второй прыжок). Если адреса назначения в
# 101 нет, ядро проваливается к следующему правилу и уходит в туннель. Поэтому
# обновление списка — это flush и заливка 101, а дефолт второго прыжка не
# трогается вообще.
set -euo pipefail
...
# NAT для прямого выхода: трафик клиентов, ушедший мимо туннеля по российским
# префиксам, обязан выходить с адреса ноды. MASQUERADE на awg1 этот путь не
# покрывает — он висит на другом интерфейсе. Правило идемпотентно, поэтому
# скрипт безопасно чинит его после перезагрузки.
...
# Правило должно стоять РАНЬШЕ правила таблицы 100, иначе дефолт во второй
# прыжок перехватит всё до того, как ядро посмотрит в российские префиксы.
...
# Управление клиентами. Скрипт называется awg и НЕ подменяет вендорскую
# утилиту: свои подкоманды он обрабатывает сам, всё остальное (show, showconf,
# set, genkey и прочее) прозрачно передаёт в /usr/bin/awg. Внутри вендорский
# бинарь зовётся по абсолютному пути — иначе скрипт вызывал бы сам себя,
# потому что /usr/local/sbin в PATH стоит раньше /usr/bin.
install_awg_cli() {
  cat > /usr/local/sbin/awg <<'EOF_AWGCLI'
#!/usr/bin/env bash
# Управление клиентами первого прыжка (awg0) поверх вендорской утилиты.
#
# Свои подкоманды: add, remove, list, names, cat, dump.
# Всё остальное уходит в /usr/bin/awg как есть, поэтому привычные
# "awg show awg0", "awg genkey" и прочее продолжают работать.
#
# Параметры обфускации и ключ защиты заголовков общие для всех клиентов — это
# свойства устройства, а не пира. Уникальны только ключи и адрес.
set -euo pipefail

# Абсолютный путь обязателен: /usr/local/sbin идёт в PATH раньше /usr/bin,
# и обычный вызов "awg" привёл бы к рекурсии в этот же скрипт.
AWG_BIN=/usr/bin/awg

# Делегируем ДО любых проверок: вендорские подкоманды не должны зависеть от
# того, лежит ли на месте наш конфиг и запущены ли мы под root.
case "${1:-}" in
    add|remove|list|names|cat|dump|help|--help|-h|"") : ;;
    *) exec "$AWG_BIN" "$@" ;;
esac
...
Свои команды:
  ...
  cat <имя>          вывести один конфиг в stdout
  dump [имя...]      tar.gz со всеми (или указанными) конфигами в stdout

Всё остальное уходит вендорской утилите: awg show, showconf, set, setconf,
addconf, syncconf, genkey, genpsk, pubkey.

Примеры:
  awg dump | tar -xzf - -C ./configs
  awg cat user01 | qrencode -t ansiutf8
  awg show awg0
USAGE
}
...
[[ -x "$AWG_BIN" ]] || die "не найден бинарь amneziawg-tools: $AWG_BIN"
...
cmd_add() {
    [[ $# -ge 1 ]] || die "укажи имя: awg add <имя> [имя2 ...]"
    ...
    for name in "$@"; do
        ...
        ip=$(next_free_ip)
        priv=$("$AWG_BIN" genkey)
        pub=$(echo "$priv" | "$AWG_BIN" pubkey)
        printf '\n[Peer]\n#_Name = %s\nPublicKey = %s\nAllowedIPs = %s/32\n' "$name" "$pub" "$ip" >> "$CONF"
        "$AWG_BIN" set "$IFACE" peer "$pub" allowed-ips "$ip/32"
        ( umask 077
...
cmd_remove() {
    [[ $# -ge 1 ]] || die "укажи имя: awg remove <имя>"
    local name pub
    for name in "$@"; do
        pub=$(peer_pubkey_by_name "$name")
        [[ -n "$pub" ]] || die "нет такого клиента: $name"
        "$AWG_BIN" set "$IFACE" peer "$pub" remove || true
...
cmd_list() {
    ...
    while read -r name ip pub; do
        hs=$("$AWG_BIN" show "$IFACE" latest-handshakes | awk -v k="$pub" '$1 == k { print $2 }')
        if [[ -z "$hs" || "$hs" == "0" ]]; then
            age="никогда"
        else
...
# cat и dump пишут в stdout и ничего больше туда не печатают — иначе поток
# нельзя было бы скормить tar или qrencode. Сообщения идут в stderr.
cmd_cat() {
    [[ $# -eq 1 ]] || die "укажи ровно одно имя: awg cat <имя>"
...
create_awg_clients() {
  ...
  /usr/local/sbin/awg add "${names[@]}" > /dev/null
  echo "Клиентов создано: $(/usr/local/sbin/awg names | wc -l)"
}

echo "=== Установка входной ноды: sing-box (hysteria2 + vless-reality) и каскад AmneziaWG ==="
...
echo
echo "Теперь каскад AmneziaWG. Скрипт выходной ноды напечатал строку вида"
echo "AWGHOP1:<base64> — в ней endpoint, ключи и параметры обфускации второго"
echo "прыжка. Клиенты будут подключаться к ЭТОЙ ноде, а за границу ходить через неё."
read -rp "Вставь строку AWGHOP1: " AWG_HOP_STRING
parse_awg_hop "$AWG_HOP_STRING"
echo "Второй прыжок: ${HOP_ENDPOINT}, адрес этой ноды в его туннеле ${HOP_IP}"

read -rp "Сколько клиентских конфигов создать? [20]: " AWG_CLIENTS
AWG_CLIENTS="${AWG_CLIENTS:-20}"
...
# Подписку отдаёт nginx, спрятанный за REALITY. Проверяем, что снаружи она
# реально открывается — иначе клиенту неоткуда взять ссылки.
sleep 1
...
# REALITY без живого dest не установит ни одного соединения, поэтому убеждаемся,
# что оттуда реально приходит TLS с сертом нашего домена.
if [[ "$VLESS_ENABLED" == "1" ]]; then
  sleep 1
...
# Каскад AmneziaWG поднимаем после sing-box: к этому моменту TCP-порты
# разобраны и видно, что UDP 443 свободен.
setup_awg_entry
setup_ru_routes
install_awg_cli
...
ufw allow 80/tcp
ufw allow 443/tcp
# AmneziaWG для клиентов. UDP 443 не конфликтует с TCP 443 у REALITY.
...
# Форвардинг нужен AmneziaWG: нода маршрутизирует трафик своих клиентов
# либо напрямую (российские префиксы), либо во второй прыжок.
...
echo
echo "--- Подписка (hysteria2:// + vless://) ---"
echo "$SUB_URL"
echo
echo "=== AmneziaWG: каскад клиент -> эта нода -> выходная нода ==="
echo "Вход для клиентов: ${AWG_PORT}/udp, подсеть ${AWG_SUBNET_PREFIX}.0/24"
echo "Второй прыжок:     ${HOP_ENDPOINT} (интерфейс awg1)"
echo "Сплит-роутинг:     $(ip route show table 101 2>/dev/null | wc -l) российских префиксов идут напрямую"
echo "Клиентов создано:  ${AWG_CLIENTS} (конфиги в /root/awg/clients)"
echo
echo "Управление клиентами на ноде:"
echo "  awg list            список"
echo "  awg add <имя>       добавить"
echo "  awg remove <имя>    удалить"
echo "  awg dump            все конфиги одним потоком"
echo "Вендорские команды не сломаны: awg show awg0 и прочее работают как раньше."
echo
