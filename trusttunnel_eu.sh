...
echo "=== Установка TrustTunnel EU exit-сервера (фикс UDP + порт 443) ==="
...
apt update
apt install -y curl ca-certificates openssl certbot python3 iproute2 ufw
...
EU_PASS=$(openssl rand -base64 24 | tr '+/' '-_' | tr -d '=' | cut -c1-28)

# Firewall — ВАЖНО: TCP + UDP
if need_cmd ufw && ufw status 2>/dev/null | grep -q "Status: active"; then
  ufw allow 80/tcp || true
  ufw allow 443/tcp || true
  ufw allow 443/udp || true
  echo "Открыли порты 443/tcp + 443/udp"
fi
...
echo "Сервер запущен. Проверяем порты..."
ss -ltnup | grep -E ':443' || echo "Порт 443 не видно (но сервис жив)"

# Генерируем правильную ссылку
TT_LINK=$(/opt/trusttunnel/trusttunnel_endpoint vpn.toml hosts.toml -c "${EU_USER}" -a "${DOMAIN}:443" --format deeplink 2>/dev/null || echo "ОШИБКА генерации")

echo
echo "=== EU-сервер ГОТОВ ==="
echo "Ссылка (только для официального TrustTunnel app):"
echo "${TT_LINK}"
echo
echo "Скачай официальное приложение TrustTunnel из App Store / Google Play и открой эту ссылку там."
